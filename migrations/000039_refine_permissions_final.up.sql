...
('VIEW_SYSTEM', 'Permissão para visualizar configurações do sistema (perfil/conta)')
...
-- 3. GESTOR: Pode criar usuários, gerenciar documentos, compartilhar e VER configurações
-- Permissões: READ, WRITE, DELETE, MANAGE_USERS, SHARE, VIEW_SYSTEM
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p 
WHERE r.name = 'GESTOR' AND p.name IN ('READ', 'WRITE', 'DELETE', 'MANAGE_USERS', 'SHARE', 'VIEW_SYSTEM');
